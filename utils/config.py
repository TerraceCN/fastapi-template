...
class NacosConfig(AbstractConfig):
    def __init__(self, host: str, username: str, password: str, dataId: str, group: str, namespace: Optional[str] = None):
        self.host = host
    ...
    def login(self, username: str, password: str):
        ...
        resp = httpx.post(
            f"{self.host}/nacos/v1/auth/login", data={"username": username, "password": password}
        )
...
class Config:
    ...
    def get(cls, name: str, default: Optional[str] = None, required: bool = True):
        ...
        values = [
            adapter.get(name, required=False) for adapter in cls.adapters
        ]
        for value in values:
